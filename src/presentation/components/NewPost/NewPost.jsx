...
const NewPost = ({ user, onPostCreated, onClose }) => {
  ...
  const removeImage = (index, target) => {
    const newImages =
      target === 'product' ? [...productImages] : [...galleryImages];
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const postData = await create(
        ...
        !isCaptionOnly && contentType === 'product'
        ...
        !isCaptionOnly && contentType === 'gallery'
          ? galleryUrls.length
          : undefined,
        !isCaptionOnly && contentType === 'gallery'
  ...
  const renderContentEditor = () => {
    switch (contentType) {
            ...
            {audioTracks.map((track, index) => (
              <div key={index} className="flex gap-2 mb-2">
                <input
                  ...
                  value={track.title}
                  onChange={e =>
                    updateAudioTrack(index, 'title', e.target.value)
                  }
          ...
          <>
            ...
            <div className="flex items-center mb-2">
              ...
              <input
                ...
                value={productPrice}
                onChange={e => {
                  const newPrice = isNaN(parseFloat(e.target.value))
                    ? 0
              ...
              {productVariants.map((variant, index) => (
                <div key={index} className="flex gap-2 mb-2">
                  ...
                  <input
                    ...
                    value={variant.value}
                    onChange={e =>
                      updateProductVariant(index, 'value', e.target.value)
                    }
                    className="flex-1 p-2 border border-gray-200 rounded-lg focus:ring-red-500 focus:border-red-500 text-gray-700"
                  />
                  <button
            ...
            {galleryImages.map((image, index) => (
              <div key={index} className="flex gap-2 mb-2">
                ...
                <input
                  ...
                  value={image.caption}
                  onChange={e =>
                    updateImageCaption(index, e.target.value, 'gallery')
                  }
                  className="flex-1 p-2 border border-gray-200 rounded-lg focus:ring-red-500 focus:border-red-500 text-gray-700"
                />
                <button
      ...
      default:
        return (
          <div className="text-red-600 text-xs">Unsupported content type</div>
        );
    }
  };
      ...
      <div className="flex mb-3">
        <div className="w-8 h-8 rounded-full overflow-hidden mr-2">
          <img
            ...
            onError={e => {
              console.warn(
                'Failed to load profile image in NewPost, using fallback'
              );
      ...
      <div className="mb-3">
        ...
        {imagePreview ? (
          ...
          <button
            ...
            className="text-red-600 text-xs mb-2 flex items-center"
          >
            <ImagePlus size={16} className="mr-1" /> Add Post Thumbnail
      ...
      {selectedTopics.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          {selectedTopics.map(topic => (
            <div
              key={topic}
              className="flex items-center bg-red-50 text-red-700 px-2 py-0.5 rounded-full text-xs"
            >
              #{topic}
