import { useTopExperts } from '@presentation/hooks/useOtherHooks';
import { useWallet } from '@presentation/hooks/useWallet';
import { useNavigate, Link } from 'react-router-dom';
import { ArrowUpRight, Search, Mail, Star } from 'lucide-react';
...
const RightSidebar = ({ rightSidebarRef, user }) => {
  ...
  const handleDirectMessage = (userId, name, rate) => {
    navigate(`/m/${userId}`, {
      state: { recipientId: userId, recipientName: name, rate },
    });
  };
  ...
  return (
    <div
      ref={rightSidebarRef}
      className="hidden lg:flex flex-col w-80 p-3 overflow-y-auto h-screen sticky top-0 border-l border-gray-200 shadow-sm"
    >
      ...
      <div className="bg-white rounded-lg p-3 border border-gray-200 shadow-sm">
            ...
            >
              <div className="flex items-center">
                <div className="w-8 h-8 rounded-full overflow-hidden mr-2 border border-red-100">
                  <img
                    src={expert.avatar_url || '/api/placeholder/32/32'}
                    alt={expert.name}
        ...
        <Link
          to="/experts"
          className="text-red-600 text-xs mt-2 inline-block hover:text-red-700"
        >
          View all experts
        </Link>
      </div>
