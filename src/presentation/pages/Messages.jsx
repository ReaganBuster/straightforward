...
import { useConversationMessages } from '@presentation/hooks/useConversationMessages';
import RenderContentCard from '@presentation/components/MessageRender/MessageRender';
...
export default function Messages({ user, onlineUsers }) {
  ...
  const { recipientId, recipientName, recipientAvatar, content } =
    location.state || {};
  ...
  useEffect(() => {
    ...
    const sendInitialMessage = async () => {
      try {
        await sendMessage(
          recipientId,
          content.data?.message || 'Started a conversation',
          null
        );
      } catch (err) {
        ...
        {[...messages].map((message, index) => (
          ...
          >
            {message.type === 'system' ? (
              ...
              <div
                className={`flex items-end space-x-2 ${message.is_current_user ? 'flex-row-reverse space-x-reverse' : ''}`}
              >
                <div
                  className={`max-w-xs md:max-w-md relative ${message.is_current_user ? 'ml-auto' : ''}`}
                >
                  {RenderContentCard ? (
                  ...
                  {message.reply_to && (
                    <div
                      className={`text-xs opacity-75 mb-2 p-2 rounded-lg ${
                  ...
                  <div
                    className={`flex items-center mt-1 space-x-2 ${message.is_current_user ? 'justify-end' : ''}`}
                  >
                    <span
                      className={`text-xs ${message.is_current_user ? 'text-red-200' : 'text-gray-400'}`}
                    >
                ...
                <div
                  className={`absolute ${message.is_current_user ? 'left-0' : 'right-0'} top-1/2 -translate-y-1/2 ${
