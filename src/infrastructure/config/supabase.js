  ...
  try {
    ...
    if (contentType) {
      const contentResponse = await supabase
        ...
        .insert({
          ...
          monetization_required:
            monetizationRequired !== undefined
              ? monetizationRequired
              : !!monetizationModel,
        })
        .select('content_id')
      ...
      switch (contentType) {
        ...
        default:
          console.warn(
            `Unknown content type: ${contentType}. No specific content table insertion performed.`
          );
      ...
      console.log(
        'No content type provided. Creating a post without specific content.'
      );
    }
  ...
  try {
    ...
    switch (contentType) {
      case 'monetized':
        ...
        break;
      case 'free':
        query = query.eq('dm_fee', 0).or('dm_fee.is.null');
        break;
      case 'all':
      default:
    ...
    if (
      ...
      posts.some(
        post =>
          ...
          post.content?.monetization_required
      )
    ) {
      ...
      const unlockedPostIds = new Set(
        unlocksData?.map(unlock => unlock.post_id) || []
      );
      const subscribedCreatorIds = new Set(
        subscriptionsData?.map(sub => sub.creator_user_id) || []
      );
      ...
      posts.forEach(post => {
        post.is_unlocked =
          ...
          (subscribedCreatorIds.has(post.user_id) &&
            post.requires_subscription) ||
          (post.dm_fee === null &&
            !post.requires_subscription &&
            !post.content?.monetization_required);
...
export const sendMessage = async (
  ...
  try {
    ...
    const { conversation_id: conversationId, rate_per_msg: ratePerMsgFromDb } =
      await startConversation(initiatorId, recipientId, ratePerMsg);
...
export const addUserFunds = async (userId, amount, description = 'Deposit') => {
  try {
    ...
    const { error: userError } = await supabase
      .from('users')
      .update({
        balance: supabase.rpc('increment', { x: amount, row: 'balance' }),
      })
      .eq('user_id', userId);
  ...
  try {
    ...
    await supabase
      ...
      .update({
        ...
        current_month_earnings: supabase.rpc('increment', {
          x: creatorAmount,
          row: 'current_month_earnings',
        }),
      })
      .eq('user_id', creatorId);
...
export const checkDmAccess = async (conversationId, userId) => {
  try {
    ...
    const otherUserId =
      convo.initiator_id === userId ? convo.recipient_id : convo.initiator_id;
...
export const requestDmAccess = async (fromId, toId, amount, postId = null) => {
  try {
    ...
    if (existingAccess) {
      ...
      const { error: insertError } = await supabase.from('dm_access').insert({
        from_user_id: fromId,
        to_user_id: toId,
        has_access: true,
        paid_amount: amount,
